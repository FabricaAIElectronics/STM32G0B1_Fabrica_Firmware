...
void SystemClock_Config(void)
{
  ...
  */
  /* Bench only (UNCOMMITTED, re-apply after any regen): this Nucleo has no
   * HSE source, so feed the identical PLL chain from HSI16/2 = the same 8 MHz
   * reference the V5.5 crystal provides. SYSCLK/PCLK1/CAN timing unchanged.
   * The generated (committed) code below this comment is HSE. */
