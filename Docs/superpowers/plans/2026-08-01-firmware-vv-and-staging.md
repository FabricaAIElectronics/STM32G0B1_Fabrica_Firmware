...
These tests reimplement the module maths against the same constants the firmware
uses, rather than compiling the firmware modules directly. `thermistor.c` and
`battery.c` both pull in HAL headers through `main.h`, and stubbing all of that
is more fragile than restating two equations. The conformance value here is that
the constants are asserted to match the firmware source, which Step 3 checks.
...
```c
/* 6S battery state-of-charge maths.
 * Firmware: STM32G0B1_Applciationprog/PowerStage/Core/Src/battery.c
 * Constants mirror Docs/CAN_Bus.md Bcast_Battery_Cfg: cutoff 19.6 V, full 25.2 V. */
#include <stdint.h>
#include "harness.h"

#define CELLS          6
#define CUTOFF_MV      19600
#define FULL_MV        25200

static uint8_t soc_from_mv(uint32_t mv)
{
    if (mv <= CUTOFF_MV) return 0;
    if (mv >= FULL_MV)   return 100;
    return (uint8_t)(((mv - CUTOFF_MV) * 100u) / (FULL_MV - CUTOFF_MV));
}

int main(void)
{
    VV_EQ_U32("soc_cell_count", CELLS, 6u);
    VV_EQ_U32("soc_at_cutoff",  soc_from_mv(CUTOFF_MV), 0u);
    VV_EQ_U32("soc_below_cutoff", soc_from_mv(18000), 0u);
    VV_EQ_U32("soc_at_full",    soc_from_mv(FULL_MV), 100u);
    VV_EQ_U32("soc_above_full", soc_from_mv(30000), 100u);
    VV_EQ_U32("soc_midpoint",   soc_from_mv((CUTOFF_MV + FULL_MV) / 2), 50u);

    /* Monotonic non-decreasing across the whole range. */
    int monotonic = 1;
    uint8_t prev = 0;
    for (uint32_t mv = CUTOFF_MV; mv <= FULL_MV; mv += 50) {
        uint8_t s = soc_from_mv(mv);
        if (s < prev) { monotonic = 0; break; }
        prev = s;
    }
    VV_CHECK("soc_monotonic", monotonic);
...
and replace the link rule so the maths tests get libm:
...
- **Logic tests restate constants rather than compiling firmware modules.**
  `thermistor.c` and `battery.c` reach HAL types through `main.h`; stubbing that
  is more fragile than restating two equations, and Task 8 Step 3 checks the
  constants against the firmware source. Compiling the real modules is a later
  improvement, which is what `vv/unit/fakes/` exists to support.
